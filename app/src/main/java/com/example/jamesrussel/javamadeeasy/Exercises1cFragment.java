 ...
 */
public class Exercises1cFragment extends Fragment {
    TextView a = null;
    TextView a1= null;
    TextView a2 = null;
    public Exercises1cFragment() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_exercises1c, container, false);

        setHasOptionsMenu(true);
        a = (TextView) view.findViewById(R.id.c1);
        a.setText(Html.fromHtml(getString(R.string.c1)));
        a1  = (TextView) view.findViewById(R.id.c2);
        a1.setText(Html.fromHtml(getString(R.string.c2)));
        a2 = (TextView) view.findViewById(R.id.c3);
        a2.setText(Html.fromHtml(getString(R.string.c3)));
        ...
        a2.setCustomSelectionActionModeCallback(new ActionMode.Callback() {
            @Override
            public boolean onCreateActionMode(ActionMode mode, Menu menu) {
                MenuInflater inflater = getActivity().getMenuInflater();
                inflater.inflate(R.menu.share_menu, menu);
            ...
            public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
                switch (item.getItemId()) {
                    case R.id.menu_item_share:
                        String selectedText = getSelectedText2();
                        shareIntent2(selectedText);
                        return true;
                    default:
                        break;
                }
    ...
    private String getSelectedText2() {
        ...
        int max = a.getText().length();
        if (a2.isFocused()) {
            final int textStartIndex = a2.getSelectionStart();
            final int textEndIndex = a2.getSelectionEnd();
            min = Math.max(0, Math.min(textStartIndex, textEndIndex));
            max = Math.max(0, Math.max(textStartIndex, textEndIndex));
            selectedText = a2.getText().subSequence(min, max).toString().trim();
        }
        return selectedText;
    }

    private void shareIntent2(String Text) {
        if (!Text.equals("")) {
            Intent shareIntent = new Intent(Intent.ACTION_SEND);
            shareIntent.setType("text/plain");
            shareIntent.putExtra(Intent.EXTRA_SUBJECT, "subject here");
